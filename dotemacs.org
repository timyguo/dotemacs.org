...
(defun add-to-load-path-include-subdirs (dir)
  "add dir and all sub-directories containining lisp code to 
 load-path.  Hijacks normal-top-level-add-subdirs-to-load-path for this" 
  (add-to-load-path dir)
  (let ((temp-default-directory default-directory))
    (when (file-exists-p dir)
      (let ((default-directory (expand-file-name dir user-emacs-directory)))
        (normal-top-level-add-subdirs-to-load-path)))
    (setq default-directory temp-default-directory)))
#+end_src
...
(setq group-dirs (org-table-to-list table))
(if (and (boundp 'group-dirs)
         group-dirs)
...
*** debug group load-path add
  #+begin_src emacs-lisp
     ...
     (debug-message "group dirs" group-dirs)
...
(setq specific-dirs (org-table-to-list table))
(if (and (boundp 'specific-dirs)
         specific-dirs)
    (mapc #'add-to-load-path
          (nreverse specific-dirs)))
#+end_src
...
:END:
  #+begin_src emacs-lisp 
    ;;(load (concat user-emacs-directory "src/nxhtml/autostart.el"))
  #+end_src
...
- Starter Kit Loads
  #+begin_src emacs-lisp
    (org-babel-load-file (expand-file-name "starter-kit-eshell.org" user-emacs-directory))
    (org-babel-load-file (expand-file-name "starter-kit-lisp.org" user-emacs-directory))
    (org-babel-load-file (expand-file-name "starter-kit-misc-recommended.org" user-emacs-directory))
  #+end_src
