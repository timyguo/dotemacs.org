...
- bootup
  - load-paths
  - usebind
  - package checks/loads
- package tweaks (alphabetical)
- other files (alphabetical)
  - gnus
  - org
  - functionality to select current directory contents
- end game
...
- loads the org directory (if it exists).  This is needed in case
  ...
  fun debugging that.    
- loads the /real/ init code, which i have called dotemacs and which
  is contained here.
